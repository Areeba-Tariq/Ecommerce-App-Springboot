...
1. Clone the repository by git bash
...
3. Make sure you are in the `JtProject` directory and allow intellij load maven dependencies and do indexing by clicking ok in dialog box
...
1. Right Click on the `JtSpringProjectApplication.java` and click edit run configuration.
2. Change the "Working directory" option (if not present, click on "Modify Options" and select from the list) to the `$MODULE_WORKING_DIR$` macro
